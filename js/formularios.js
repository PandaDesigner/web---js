...
const methFormulario ={
    inicio: function () {
        for (const index in propFormulario.formElementos) {
           if(propFormulario.formElementos[index].type == 'text' || propFormulario.formElementos[index].type == 'email' || propFormulario.formElementos[index].type == 'textarea'){
            propFormulario.formElementos[index].addEventListener('focus', focusInput)
    ...
    validarInput: (e) =>{
        for (const index in propFormulario.formElementos) {
            if(propFormulario.formElementos[index].value == ''){
                ...
                if(propFormulario.formElementos[index].parentElement.children.length < 3 ){
